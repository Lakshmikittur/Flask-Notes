...
def createnote():
    if "username" in request.values and "password" in request.values and "title" in request.values and "content" in request.values:
        if accounts_common_helpers.check_username_password(request.values["username"], request.values["password"]):
            ...
            newnote = Note(author_id = tempUser.id, title = request.values["title"], content = request.values["content"])
...
def editnote():
    if "username" in request.values and "password" in request.values and "title" in request.values and "noteid" in request.values and "content" in request.values:
        ...
        if user_valid:
            ...
            if notes_common_helpers.checknoteidbyauthorid(tempUser.id, request.values["noteid"]):
                ...
                tempnote = notes_common_helpers.editnote( request.values["noteid"], content, title)            
                return dmp(True)
            else:
